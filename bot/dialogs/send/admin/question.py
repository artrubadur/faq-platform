...
import bot.dialogs.markups.question as mu
...
async def send_found_similar(
    ...
    return await send(
        text=messages.responses.admin.question.creation.similar.format(
...
async def send_confirm_deletion(
    ...
    warning = ""
    if formulation_ids:
        formulation_ids_text = ", ".join(
            format_id(formulation_id) for formulation_id in formulation_ids
        )
        warning = (
            "\n"
            + messages.responses.admin.question.deletion.formulations_warning.format(
                amount=len(formulation_ids),
                ids=formulation_ids_text,
            )
        )

    return await send(
        text=(
            messages.responses.admin.question.deletion.confirm.format(
                question=format_question(id, question_text, answer_text)
            )
            + warning
        ),
...
async def send_pagination(
    ...
    columns = [
        name
        for name in QuestionResponse.model_fields
        if name not in {"answer_text", "formulation_ids"}
    ]
